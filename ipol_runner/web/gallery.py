"""Gallery and comparison views for IPOL Runner web UI."""
import json
...
CATEGORY_STYLES = {
    "denoising": {"color": "#4CAF50", "icon": "🧹", "label": "Denoising"},
    "change_detection": {"color": "#2196F3", "icon": "🔍", "label": "Change Detection"},
    "detection": {"color": "#FF9800", "icon": "🎯", "label": "Detection"},
    "segmentation": {"color": "#9C27B0", "icon": "✂️", "label": "Segmentation"},
    "generation": {"color": "#E91E63", "icon": "🎨", "label": "Generation"},
    "3d_reconstruction": {"color": "#00BCD4", "icon": "🏗️", "label": "3D Reconstruction"},
    "phase_processing": {"color": "#607D8B", "icon": "🌊", "label": "Phase Processing"},
    "medical": {"color": "#F44336", "icon": "🏥", "label": "Medical"},
}
...
def get_gallery_html(embedded_data: Optional[dict] = None):
    """Generate the gallery HTML page.

    Args:
        embedded_data: If provided, embed method data directly in HTML for standalone mode.
                      Format: {"methods": [...], "validation": {...}}
    """
    # JavaScript to handle embedded vs API data
    if embedded_data:
        data_script = f"""
    ...
    <script>
        ...
        function setupFilters() {
            ...
            categories.forEach(cat => {
                const style = CATEGORY_STYLES[cat] || { icon: '📦', label: cat };
            ...
            inputs.forEach(inp => {
                const icon = INPUT_TYPE_ICONS[inp] || '📄';
                const btn = document.createElement('button');
                btn.className = 'filter-btn';
        ...
        function renderGallery() {
            ...
            content.innerHTML = Object.entries(byCategory).map(([cat, catMethods]) => {
                const style = CATEGORY_STYLES[cat] || { color: '#666', icon: '📦', label: cat };
                return `
        ...
        function renderMethodCard(method, accentColor) {
            const inputIcon = INPUT_TYPE_ICONS[method.input_type] || '📄';
            ...
            let statusBadge = '';
            if (passed) statusBadge = '<span class="status-badge passed">✓ Validated</span>';
            else if (failed) statusBadge = '<span class="status-badge failed">✗ Failed</span>';
            ...
            if (STANDALONE_MODE) {
                ...
                body.innerHTML = `
                    <div class="method-info">
                        ...
                        ${passed ? `<p><strong>Status:</strong> <span class="status-badge passed">✓ Validated</span></p>
                            <p><strong>Output:</strong> ${passed.output || 'N/A'}</p>
                            ${passed.notes ? `<p><strong>Notes:</strong> ${passed.notes}</p>` : ''}` : ''}
                        ${failed ? `<p><strong>Status:</strong> <span class="status-badge failed">✗ Failed</span></p>
        ...
        async function handleFiles(files) {
            ...
            fileList.innerHTML = uploadedFiles.map(f => `<p>✅ ${f.filename}</p>`).join('');
        }
                        ...
                        <div class="result-card">
                            ...
                            <div class="result-info">
                                ...
                                <div class="meta">
                                    ...
                                    ${e.execution_time ? ` • ${e.execution_time.toFixed(1)}s` : ''}
                                </div>
                            </div>
                        </div>
                    `).join('')}
...
def generate_standalone_gallery(output_path: Optional[Path] = None) -> str:
    """Generate a standalone HTML gallery with embedded method data.
    ...
    validation = {}
    status_file = P(__file__).parent.parent.parent / "validation_status.json"
    ...
    embedded_data = {
        "methods": sorted(methods_list, key=lambda m: m["name"]),
        "validation": validation
    }

    html = get_gallery_html(embedded_data)
